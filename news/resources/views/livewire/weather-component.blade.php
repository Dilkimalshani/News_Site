                    ...
                    <div class="position-relative mb-3">
                        <div class="bg-white border border-top-0 p-4">
                            @foreach ($posts as $post)
                                <div class="mb-3">
                                    <a class="badge badge-primary text-uppercase font-weight-semi-bold p-2 mr-2" href="">Weather</a>
                                    <a class="text-body" href="">{{ $post->created_at }}</a>
                                </div>
                                <h1 class="mb-3 text-uppercase font-weight-bold" style="color: #000000; font-weight: bold;">{{ $post->title }}</h1>
                                <p>{{ $post->short_desc }}</p>
                                <img class="img-fluid w-50 float-left mr-4 mb-2" src="{{ asset('asset/img/img3.jpg') }}">
                                <p>{{ $post->long_desc }}</p>
                            @endforeach
                        </div>
                    </div>
                            ...
                            @foreach ($breaking_news as $breaking_new)
                                <div class="d-flex align-items-center bg-white mb-3" style="height: 110px;">
                                    ...
                                    <div class="w-100 h-100 px-3 d-flex flex-column justify-content-center border border-left-0">
                                        <div class="mb-2">
                                            <a class="badge badge-primary text-uppercase font-weight-semi-bold p-1 mr-2" href="">Weather</a>
                                            <a class="text-body" href="">{{ $breaking_new->published_date }}</a>
                                        </div>
                                        <a class="h6 m-0 text-secondary text-uppercase font-weight-bold" href="">{{ $breaking_new->title }}</a>
                                    </div>
                                </div>
                            @endforeach
                        </div>
